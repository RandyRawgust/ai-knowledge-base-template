...
function formatValueForChip(val) {
  var _a;
  if (val instanceof import_obsidian4.DateValue) {
    return val.relative();
  }
  if (val instanceof import_obsidian4.LinkValue) {
  ...
  if (val instanceof import_obsidian4.ListValue) {
    const parts = [];
    const len = val.length();
    for (let i = 0; i < len; i++) {
      const item = val.get(i);
      if (!item || item instanceof import_obsidian4.NullValue || !item.isTruthy()) continue;
...
var CardManager = class {
  ...
  renderCard(cardsEl, entry, columnName) {
    ...
    if (titleProp) {
      ...
      if (tv && !(tv instanceof import_obsidian4.NullValue) && tv.isTruthy()) {
